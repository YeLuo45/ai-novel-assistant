...
export default function WritingStatsDashboard({ projectId }: Props) {
  ...
  return (
    <div className="p-6 bg-dark-bg-tertiary min-h-screen">
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-bold text-dark-text">写作统计</h1>
          
          {/* View Mode Tabs */}
          <div className="flex bg-dark-bg rounded-lg p-1 shadow-sm">
            <button
              onClick={() => setViewMode('overview')}
              className={`px-4 py-2 text-sm font-medium rounded-lg transition-colors ${
                viewMode === 'overview' ? 'bg-indigo-100 text-indigo-700' : 'text-dark-text-secondary hover:bg-dark-bg-tertiary'
              }`}
            >
              总览
            </button>
            <button
              ...
              className={`px-4 py-2 text-sm font-medium rounded-lg transition-colors ${
                viewMode === 'detailed' ? 'bg-indigo-100 text-indigo-700' : 'text-dark-text-secondary hover:bg-dark-bg-tertiary'
              }`}
            >
              详细分析
            </button>
            <button
              ...
              className={`px-4 py-2 text-sm font-medium rounded-lg transition-colors ${
                viewMode === 'characters' ? 'bg-indigo-100 text-indigo-700' : 'text-dark-text-secondary hover:bg-dark-bg-tertiary'
              }`}
            >
              角色统计
            </button>
            <button
              ...
              className={`px-4 py-2 text-sm font-medium rounded-lg transition-colors ${
                viewMode === 'heatmap' ? 'bg-indigo-100 text-indigo-700' : 'text-dark-text-secondary hover:bg-dark-bg-tertiary'
              }`}
            >
              热力图
            </button>
        ...
        {viewMode === 'overview' && (
          <>
            ...
            <div className="grid grid-cols-4 gap-4 mb-6">
              <div className="bg-dark-bg rounded-xl p-4 shadow-sm">
                <div className="text-sm text-dark-text-secondary mb-1">今日字数</div>
                <div className="text-2xl font-bold text-indigo-600">
                  {writingStats.length > 0 ? writingStats[writingStats.length - 1].wordCount.toLocaleString() : 0}
                </div>
              </div>
              <div className="bg-dark-bg rounded-xl p-4 shadow-sm">
                <div className="text-sm text-dark-text-secondary mb-1">今日目标</div>
                <div className="text-2xl font-bold text-dark-text">{dailyGoal.toLocaleString()}</div>
              </div>
              <div className="bg-dark-bg rounded-xl p-4 shadow-sm">
                <div className="text-sm text-dark-text-secondary mb-1">全书字数</div>
                <div className="text-2xl font-bold text-purple-600">{totalWordCount.toLocaleString()}</div>
              </div>
              <div className="bg-dark-bg rounded-xl p-4 shadow-sm">
                <div className="text-sm text-dark-text-secondary mb-1">完成进度</div>
            ...
            <div className="grid grid-cols-2 gap-6 mb-6">
              ...
              <div className="bg-dark-bg rounded-xl p-6 shadow-sm">
                <h3 className="text-lg font-semibold text-dark-text mb-4">近7天写作字数</h3>
              ...
              <div className="bg-dark-bg rounded-xl p-6 shadow-sm">
                <h3 className="text-lg font-semibold text-dark-text mb-4">全书完成度</h3>
                <ResponsiveContainer width="100%" height={250}>
                  <PieChart>
                    <Pie
                      data={donutData}
                ...
                <div className="text-center mt-2">
                  <span className="text-3xl font-bold text-dark-text">{bookCompletion}%</span>
                  <p className="text-sm text-dark-text-secondary">全书完成度</p>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <div className="bg-dark-bg rounded-xl p-6 shadow-sm">
                <h3 className="text-lg font-semibold text-dark-text mb-4">章节字数分布</h3>
                <ResponsiveContainer width="100%" height={250}>
              ...
              <div className="bg-dark-bg rounded-xl p-6 shadow-sm">
                <h3 className="text-lg font-semibold text-dark-text mb-4">章节详情</h3>
                <div className="overflow-y-auto max-h-56">
                  <table className="w-full text-sm">
                    <thead className="bg-dark-bg-tertiary sticky top-0">
                      <tr>
                        <th className="text-left py-2 px-2 text-dark-text-secondary font-medium">章节</th>
                        <th className="text-right py-2 px-2 text-dark-text-secondary font-medium">字数</th>
                        <th className="text-right py-2 px-2 text-dark-text-secondary font-medium">进度</th>
                      </tr>
                    </thead>
                    <tbody>
                      {chapterStats.map((stat, index) => (
                        <tr key={index} className="border-t border-dark-border">
                          <td className="py-2 px-2 text-dark-text truncate max-w-[150px]">{stat.title}</td>
                          <td className="py-2 px-2 text-right text-dark-text-secondary">{stat.wordCount.toLocaleString()}</td>
                          <td className="py-2 px-2 text-right">
                            <div className="flex items-center justify-end gap-2">
                              ...
                              <span className="text-xs text-dark-text-secondary">{stat.progress}%</span>
                            </div>
                          </td>
                        </tr>
                      ))}
                      {chapterStats.length === 0 && (
                        <tr>
                          <td colSpan={3} className="py-4 text-center text-dark-text-secondary">暂无章节数据</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
        ...
        {viewMode === 'detailed' && (
          <>
            ...
            <div className="grid grid-cols-3 gap-4 mb-6">
              <div className="bg-dark-bg rounded-xl p-4 shadow-sm">
                <div className="text-sm text-dark-text-secondary mb-1">连续写作天数</div>
                <div className="text-2xl font-bold text-orange-600">{streak}</div>
              </div>
              <div className="bg-dark-bg rounded-xl p-4 shadow-sm">
                <div className="text-sm text-dark-text-secondary mb-1">章节总数</div>
                <div className="text-2xl font-bold text-blue-600">{outlineNodes.filter(n => n.type === 'chapter').length}</div>
              </div>
              <div className="bg-dark-bg rounded-xl p-4 shadow-sm">
                <div className="text-sm text-dark-text-secondary mb-1">已完成章节</div>
            ...
            <div className="bg-dark-bg rounded-xl p-6 shadow-sm mb-6">
              <h3 className="text-lg font-semibold text-dark-text mb-4">30天写作趋势</h3>
              <ResponsiveContainer width="100%" height={250}>
            ...
            <div className="bg-dark-bg rounded-xl p-6 shadow-sm">
              <h3 className="text-lg font-semibold text-dark-text mb-4">项目结构</h3>
              <div className="grid grid-cols-4 gap-4">
                <div className="text-center p-3 bg-dark-bg-tertiary rounded-lg">
                  <div className="text-2xl font-bold text-indigo-600">
                    {outlineNodes.filter(n => n.type === 'volume').length}
                  </div>
                  <div className="text-sm text-dark-text-secondary">卷</div>
                </div>
                <div className="text-center p-3 bg-dark-bg-tertiary rounded-lg">
                  <div className="text-2xl font-bold text-purple-600">
                    {outlineNodes.filter(n => n.type === 'chapter').length}
                  </div>
                  <div className="text-sm text-dark-text-secondary">章</div>
                </div>
                <div className="text-center p-3 bg-dark-bg-tertiary rounded-lg">
                  <div className="text-2xl font-bold text-blue-600">
                    {outlineNodes.filter(n => n.type === 'section').length}
                  </div>
                  <div className="text-sm text-dark-text-secondary">节</div>
                </div>
                <div className="text-center p-3 bg-dark-bg-tertiary rounded-lg">
                  <div className="text-2xl font-bold text-green-600">
                    {outlineNodes.filter(n => n.type === 'scene').length}
                  </div>
                  <div className="text-sm text-dark-text-secondary">场景</div>
                </div>
              </div>
            </div>
          </>
        ...
        {viewMode === 'characters' && (
          <>
            ...
            <div className="grid grid-cols-3 gap-4 mb-6">
              <div className="bg-dark-bg rounded-xl p-4 shadow-sm">
                <div className="text-sm text-dark-text-secondary mb-1">角色总数</div>
                <div className="text-2xl font-bold text-indigo-600">{characterStats.totalCharacters}</div>
              </div>
              <div className="bg-dark-bg rounded-xl p-4 shadow-sm">
                <div className="text-sm text-dark-text-secondary mb-1">活跃角色</div>
                <div className="text-2xl font-bold text-green-600">{characterStats.activeCharacters}</div>
              </div>
              <div className="bg-dark-bg rounded-xl p-4 shadow-sm">
                <div className="text-sm text-dark-text-secondary mb-1">角色关系数</div>
            ...
            <div className="bg-dark-bg rounded-xl p-6 shadow-sm">
              <h3 className="text-lg font-semibold text-dark-text mb-4">角色列表</h3>
              <div className="grid grid-cols-3 gap-4">
                {materialCards.filter(c => c.type === 'character').map(card => (
<div className="p-4 bg-dark-bg-tertiary rounded-lg border border-dark-border hover:border-indigo-200 transition-colors">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <div>
                        <div className="font-medium text-dark-text">{card.name}</div>
                        {card.fields?.role && (
                          <div className="text-xs text-dark-text-secondary">{card.fields.role}</div>
                        )}
                      </div>
                    </div>
                    {card.fields?.description && (
                      <p className="text-sm text-dark-text-secondary line-clamp-2">{card.fields.description}</p>
                    )}
                  </div>
                ))}
                {materialCards.filter(c => c.type === 'character').length === 0 && (
                  <div className="col-span-3 py-8 text-center text-dark-text-secondary">
                    暂无角色素材卡
                  </div>
                )}
              </div>
