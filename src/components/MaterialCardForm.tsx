...
export function MaterialCardForm({ card, type, onSave, onCancel }: MaterialCardFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3">
      <div>
        <label className="block text-sm font-medium text-dark-text dark:text-gray-300 mb-1">
          名称
        </label>
        <input
          type="text"
          ...
          className="w-full px-3 py-2 border border-dark-border dark:border-gray-600 rounded-lg bg-dark-bg dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent"
        ...
        <div>
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              className="flex-1 px-3 py-2 border border-dark-border dark:border-gray-600 rounded-lg bg-dark-bg dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500"
          ...
          {avatar && (
            <div className="mt-2 flex items-center gap-2">
              <img
                src={avatar}
                alt="头像预览"
                className="w-12 h-12 rounded-full border-2 border-dark-border object-cover bg-gray-100"
      ...
      {currentProjectId && (
        <div>
          ...
          <button
            ...
            className="w-full px-3 py-2 border border-dark-border dark:border-gray-600 rounded-lg bg-dark-bg dark:bg-gray-700 text-gray-900 dark:text-white text-left text-sm flex items-center justify-between"
          >
          ...
          {showTagSelector && availableTags.length > 0 && (
            <div className="mt-1 border border-dark-border dark:border-gray-600 rounded-lg bg-dark-bg dark:bg-gray-700 max-h-40 overflow-y-auto">
              {availableTags.map(tag => (
                <button
                  key={tag.id}
                  type="button"
                  onClick={() => toggleTag(String(tag.id))}
                  className={`w-full px-3 py-2 text-left text-sm flex items-center gap-2 hover:bg-dark-bg-tertiary dark:hover:bg-gray-600 ${
      ...
      <div className="flex gap-2 pt-2">
        ...
        <button
          type="button"
          onClick={onCancel}
          className="flex-1 px-4 py-2 bg-gray-200 dark:bg-gray-600 text-dark-text dark:text-gray-200 rounded-lg hover:bg-gray-300 dark:hover:bg-dark-bg-tertiary transition-colors"
        >
          取消
        </button>
      </div>
