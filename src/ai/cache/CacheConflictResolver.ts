// V2260 CacheConflictResolver - Direction I Iter 25/30
// LWW/CRDT cache conflict resolver
// Source: chatdev
export type CacheConflictStrategy = 'lww' | 'crdt';
...
export interface CacheResolution {
  winner: CacheVersion;
  losers: CacheVersion[];
  strategy: CacheConflictStrategy;
  resolvedAt: number;
}

export interface CacheConflictState {
  history: CacheResolution[];
}

export function createCacheConflictState(): CacheConflictState {
  return { history: [] };
}

export function resolveCacheLWW(conflicts: CacheVersion[]): CacheResolution {
...
export function resolveCacheCRDT(conflicts: CacheVersion[]): CacheResolution {
  if (conflicts.length === 0) throw new Error('at least 1 version required');
  const sorted = [...conflicts].sort((a, b) => b.ts - a.ts);
...
export function recordCacheResolution(state: CacheConflictState, resolution: CacheResolution): CacheConflictState {
  return { ...state, history: [...state.history, resolution] };
}

export function detectCacheConflict(versions: CacheVersion[]): boolean {
  if (versions.length < 2) return false;
  return new Set(versions.map((v) => v.authorId)).size > 1;
}

export function cacheConflictHealth(state: CacheConflictState): { resolved: number; lww: number; crdt: number; health: number } {
